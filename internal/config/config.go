...
type Config struct {
	...
	Path               string            `toml:"-"`
	AccurankerApiToken string            `toml:"api_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("ACCURANKER_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "accuranker-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("ACCURANKER_API_TOKEN"); v != "" {
		cfg.AccurankerApiToken = v
		cfg.AuthSource = "env:ACCURANKER_API_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.AccurankerApiToken != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("ACCURANKER_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.AccurankerApiToken
	if token == "" {
		return ""
	}
	if c.AccurankerApiToken == "" {
		return ""
	}
	replacements := map[string]string{
		"token":                token,
		"api_token":            c.AccurankerApiToken,
		"ACCURANKER_API_TOKEN": c.AccurankerApiToken,
	}
	return applyAuthFormat("Token {token}", replacements)
}
