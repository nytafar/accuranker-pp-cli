...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  accuranker-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  accuranker-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// When the workflow's own --json envelope is requested, the
			// underlying syncResource still writes per-resource NDJSON
			// `sync_*` events to stdout (sync.go is package-level and
			// reads the `humanFriendly` flag). Without this guard, the
			// stdout stream becomes "event-line + event-line + ... +
			// workflow-envelope" which is not parseable as a single JSON
			// document. Toggling `humanFriendly=true` for the duration
			// routes sync's status messages to stderr, leaving stdout
			// clean for the workflow's own JSON output. Restored on
			// return so other commands (e.g. the standalone `sync`) are
			// unaffected.
			if flags.asJSON {
			...
			resources := []string{"accounts", "brands", "domains"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  accuranker-pp-cli workflow status

  # Show status as JSON
  accuranker-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("accuranker-pp-cli")
			}
