...
Resource scoping:
  ...
  accuranker-pp-cli sync

  # Sync specific resources only
  accuranker-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  accuranker-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  accuranker-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  accuranker-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  accuranker-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("accuranker-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/accuranker-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "brand":
		return db.UpsertBrand(data)
	case "prompts":
		return db.UpsertPrompts(data)
	case "domain":
		return db.UpsertDomain(data)
	case "domains":
		return db.UpsertDomains(data)
	case "keywords":
		return db.UpsertKeywords(data)
	case "landing_pages":
		return db.UpsertLandingPages(data)
	case "tags":
		return db.UpsertTags(data)
	case "group":
		return db.UpsertGroup(data)
	case "keyword":
		return db.UpsertKeyword(data)
	case "overview":
		return db.UpsertOverview(data)
	case "prompt":
		return db.UpsertPrompt(data)
	case "status":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"brands":   "/api/v4/brands/",
		"domains":  "/api/v4/domains/",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "keywords", ParentTable: "domains", ParentIDParam: "domain_pk", PathTemplate: "/api/v4/domains/{domain_pk}/keywords/", KeyField: ""},
		{Name: "landing_pages", ParentTable: "domains", ParentIDParam: "domain_pk", PathTemplate: "/api/v4/domains/{domain_pk}/landing_pages/", KeyField: ""},
		{Name: "prompts", ParentTable: "brands", ParentIDParam: "brands_pk", PathTemplate: "/api/v4/brands/{brands_pk}/prompts/", KeyField: ""},
		{Name: "tags", ParentTable: "domains", ParentIDParam: "domain_pk", PathTemplate: "/api/v4/domains/{domain_pk}/tags/", KeyField: ""},
	}
}
