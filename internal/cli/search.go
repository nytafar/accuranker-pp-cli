...
In local mode: searches locally synced data only.`,
  ...
  accuranker-pp-cli search "error timeout"

  # Force local search only
  accuranker-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  accuranker-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  accuranker-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /api/v4/domains/{domain_pk}/keywords/{keyword_pk}/ranks/{search_date}/
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/api/v4/domains/{domain_pk}/keywords/{keyword_pk}/ranks/{search_date}/", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'accuranker-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "group":
				results, err = db.SearchGroup(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchGroup(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search group failed: %w", searchErr)
					}
