...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".accuranker-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ACCURANKER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ACCURANKER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "accuranker-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.accuranker-pp-cli/feedback.jsonl.
When ` + "`ACCURANKER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ACCURANKER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  accuranker-pp-cli feedback list
  accuranker-pp-cli feedback list --limit 5
  accuranker-pp-cli feedback list --json`,
