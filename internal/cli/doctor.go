...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  accuranker-pp-cli doctor
  accuranker-pp-cli doctor --json
  accuranker-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export ACCURANKER_API_TOKEN=<your-key>"
					report["auth_key_url"] = "https://app.accuranker.com/app/profile"
				} else {
					authConfigured = true
			...
			if os.Getenv("ACCURANKER_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "ACCURANKER_API_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ACCURANKER_API_TOKEN")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'accuranker-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'accuranker-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'accuranker-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'accuranker-pp-cli sync' to refresh."
	}
	return report
}
